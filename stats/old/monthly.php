...
foreach ($pld_lines as $line) {
	for ($month=1; $month<=12; $month++) {
		...
		if ((int)$month < 12)
			$q = @$DBhandle->query('select count(*) from application where line="' . $line . '" and date >="2009-' . (string)$month . '-01 00:00:00" and date < "2009-'. (string)$month2 .'-01 00:00:00"');
		else
			$q = @$DBhandle->query('select count(*) from application where line="' . $line . '" and date >="2009-' . (string)$month . '-01 00:00:00"');
		if ($line == "th")
...
$title = new title("2009 Monthly Requests");
